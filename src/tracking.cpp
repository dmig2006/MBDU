...
//    TODO: принимаем, что после получения второй опорной точки ПП "моментально" предвигается в расчётную точку встречи
...
//    rN1, rN2 задаётся в метрах, пересчитать исходя из заданной дальности в углы
    ...
    mMutex.unlock();
//    время моторов на перемещение; в ПУУ и регуляторе А.А. как бы равномерное движение
//     V_mot и A_mot скорость и ускорение перемещения соответственно
    ...
    if (tVx < 0 || tVy < 0)
        return false;
//    заменить tVx и tVy на максимальное значение!
//    смещение цели за время полёта тела и время движения ПП + смещение на баллистику
//    tEx и tEy время на отротку команд (погрешность)y_target
    float max_time = 0;
    if (tVx > tVy)
        max_time = tVx;
    else
...
//    по Х пределов нет, но ставим 180 градусов
